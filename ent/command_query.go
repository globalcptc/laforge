...
type CommandQuery struct {
	...
	withNamedCommandToUser   map[string]*UserQuery
...
func (cq *CommandQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Command, error) {
	...
	for name, query := range cq.withNamedCommandToUser {
		if err := cq.loadCommandToUser(ctx, query, nodes,
			func(n *Command) { n.appendNamedCommandToUser(name) },
			func(n *Command, e *User) { n.appendNamedCommandToUser(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (cq *CommandQuery) loadCommandToUser(ctx context.Context, query *UserQuery, nodes []*Command, init func(*Command), assign func(*Command, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(command.CommandToUserColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "command_command_to_user" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
// WithNamedCommandToUser tells the query-builder to eager-load the nodes that are connected to the "CommandToUser"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (cq *CommandQuery) WithNamedCommandToUser(name string, opts ...func(*UserQuery)) *CommandQuery {
	query := (&UserClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if cq.withNamedCommandToUser == nil {
		cq.withNamedCommandToUser = make(map[string]*UserQuery)
	}
	cq.withNamedCommandToUser[name] = query
	return cq
}
