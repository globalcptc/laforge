...
func (d *DNS) NamedEnvironments(name string) ([]*Environment, error) {
	if d.Edges.namedEnvironments == nil {
		return nil, &NotLoadedError{edge: name}
	}
	nodes, ok := d.Edges.namedEnvironments[name]
	if !ok {
		return nil, &NotLoadedError{edge: name}
	}
	return nodes, nil
...
func (d *DNS) appendNamedEnvironments(name string, edges ...*Environment) {
	if d.Edges.namedEnvironments == nil {
		d.Edges.namedEnvironments = make(map[string][]*Environment)
	}
	if len(edges) == 0 {
		d.Edges.namedEnvironments[name] = []*Environment{}
	} else {
		d.Edges.namedEnvironments[name] = append(d.Edges.namedEnvironments[name], edges...)
	}
}

// NamedCompetitions returns the Competitions named value or an error if the edge was not
// loaded in eager-loading with this name.
func (d *DNS) NamedCompetitions(name string) ([]*Competition, error) {
	if d.Edges.namedCompetitions == nil {
		return nil, &NotLoadedError{edge: name}
	}
	nodes, ok := d.Edges.namedCompetitions[name]
	if !ok {
		return nil, &NotLoadedError{edge: name}
	}
	return nodes, nil
...
func (d *DNS) appendNamedCompetitions(name string, edges ...*Competition) {
	if d.Edges.namedCompetitions == nil {
		d.Edges.namedCompetitions = make(map[string][]*Competition)
	}
	if len(edges) == 0 {
		d.Edges.namedCompetitions[name] = []*Competition{}
	} else {
		d.Edges.namedCompetitions[name] = append(d.Edges.namedCompetitions[name], edges...)
	}
}
