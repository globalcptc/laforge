...
type DNSEdges struct {
	...
	namedDNSToEnvironment map[string][]*Environment
	namedDNSToCompetition map[string][]*Competition
}
...
func (*DNS) scanValues(columns []string) ([]any, error) {
...
// to the DNS fields.
func (d *DNS) assignValues(columns []string, values []any) error {
...
// Value returns the ent.Value that was dynamically selected and assigned to the DNS.
// This includes values selected through modifiers, order, etc.
func (d *DNS) Value(name string) (ent.Value, error) {
...
func (d *DNS) QueryDNSToEnvironment() *EnvironmentQuery {
	return NewDNSClient(d.config).QueryDNSToEnvironment(d)
}
...
func (d *DNS) QueryDNSToCompetition() *CompetitionQuery {
	return NewDNSClient(d.config).QueryDNSToCompetition(d)
}
...
func (d *DNS) Update() *DNSUpdateOne {
	return NewDNSClient(d.config).UpdateOne(d)
}
...
// NamedDNSToEnvironment returns the DNSToEnvironment named value or an error if the edge was not
// loaded in eager-loading with this name.
func (d *DNS) NamedDNSToEnvironment(name string) ([]*Environment, error) {
	if d.Edges.namedDNSToEnvironment == nil {
		return nil, &NotLoadedError{edge: name}
	}
	nodes, ok := d.Edges.namedDNSToEnvironment[name]
	if !ok {
		return nil, &NotLoadedError{edge: name}
	}
	return nodes, nil
...
func (d *DNS) appendNamedDNSToEnvironment(name string, edges ...*Environment) {
	if d.Edges.namedDNSToEnvironment == nil {
		d.Edges.namedDNSToEnvironment = make(map[string][]*Environment)
	}
	if len(edges) == 0 {
		d.Edges.namedDNSToEnvironment[name] = []*Environment{}
	} else {
		d.Edges.namedDNSToEnvironment[name] = append(d.Edges.namedDNSToEnvironment[name], edges...)
	}
}

// NamedDNSToCompetition returns the DNSToCompetition named value or an error if the edge was not
// loaded in eager-loading with this name.
func (d *DNS) NamedDNSToCompetition(name string) ([]*Competition, error) {
	if d.Edges.namedDNSToCompetition == nil {
		return nil, &NotLoadedError{edge: name}
	}
	nodes, ok := d.Edges.namedDNSToCompetition[name]
	if !ok {
		return nil, &NotLoadedError{edge: name}
	}
	return nodes, nil
...
func (d *DNS) appendNamedDNSToCompetition(name string, edges ...*Competition) {
	if d.Edges.namedDNSToCompetition == nil {
		d.Edges.namedDNSToCompetition = make(map[string][]*Competition)
	}
	if len(edges) == 0 {
		d.Edges.namedDNSToCompetition[name] = []*Competition{}
	} else {
		d.Edges.namedDNSToCompetition[name] = append(d.Edges.namedDNSToCompetition[name], edges...)
	}
}
