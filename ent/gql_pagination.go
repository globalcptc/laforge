...
func (c *ProvisioningScheduledStepConnection) build(nodes []*ProvisioningScheduledStep, pager *provisioningscheduledstepPager, after *Cursor, first *int, before *Cursor, last *int) {
	c.PageInfo.HasNextPage = before != nil
	c.PageInfo.HasPreviousPage = after != nil
	if first != nil && *first+1 == len(nodes) {
		c.PageInfo.HasNextPage = true
	...
	c.Edges = make([]*ProvisioningScheduledStepEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &ProvisioningScheduledStepEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
func newProvisioningScheduledStepPager(opts []ProvisioningScheduledStepPaginateOption, reverse bool) (*provisioningscheduledstepPager, error) {
	pager := &provisioningscheduledstepPager{reverse: reverse}
	for _, opt := range opts {
		if err := opt(pager); err != nil {
			return nil, err
		}
...
func (p *provisioningscheduledstepPager) applyCursors(query *ProvisioningScheduledStepQuery, after, before *Cursor) (*ProvisioningScheduledStepQuery, error) {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	for _, predicate := range entgql.CursorsPredicate(after, before, DefaultProvisioningScheduledStepOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}

func (p *provisioningscheduledstepPager) applyOrder(query *ProvisioningScheduledStepQuery) *ProvisioningScheduledStepQuery {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	if p.order.Field != DefaultProvisioningScheduledStepOrder.Field {
		query = query.Order(DefaultProvisioningScheduledStepOrder.Field.toTerm(direction.OrderTermOption()))
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(p.order.Field.column)
	}
...
func (p *provisioningscheduledstepPager) orderExpr(query *ProvisioningScheduledStepQuery) sql.Querier {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.column).Pad().WriteString(string(direction))
		if p.order.Field != DefaultProvisioningScheduledStepOrder.Field {
			b.Comma().Ident(DefaultProvisioningScheduledStepOrder.Field.column).Pad().WriteString(string(direction))
		}
	})
}
...
func (pss *ProvisioningScheduledStepQuery) Paginate(
	...
	pager, err := newProvisioningScheduledStepPager(opts, last != nil)
	if err != nil {
		return nil, err
	}
...
type ProvisioningScheduledStepOrderField struct {
	// Value extracts the ordering value from the given ProvisioningScheduledStep.
	Value    func(*ProvisioningScheduledStep) (ent.Value, error)
	column   string // field or computed.
	toTerm   func(...sql.OrderTermOption) provisioningscheduledstep.OrderOption
...
var DefaultProvisioningScheduledStepOrder = &ProvisioningScheduledStepOrder{
	...
	Field: &ProvisioningScheduledStepOrderField{
		Value: func(pss *ProvisioningScheduledStep) (ent.Value, error) {
			return pss.ID, nil
		},
		column: provisioningscheduledstep.FieldID,
		toTerm: provisioningscheduledstep.ByID,
...
func (c *ScheduledStepConnection) build(nodes []*ScheduledStep, pager *scheduledstepPager, after *Cursor, first *int, before *Cursor, last *int) {
	c.PageInfo.HasNextPage = before != nil
	c.PageInfo.HasPreviousPage = after != nil
	if first != nil && *first+1 == len(nodes) {
		c.PageInfo.HasNextPage = true
	...
	c.Edges = make([]*ScheduledStepEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &ScheduledStepEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
func newScheduledStepPager(opts []ScheduledStepPaginateOption, reverse bool) (*scheduledstepPager, error) {
	pager := &scheduledstepPager{reverse: reverse}
	for _, opt := range opts {
		if err := opt(pager); err != nil {
			return nil, err
		}
...
func (p *scheduledstepPager) applyCursors(query *ScheduledStepQuery, after, before *Cursor) (*ScheduledStepQuery, error) {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	for _, predicate := range entgql.CursorsPredicate(after, before, DefaultScheduledStepOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}

func (p *scheduledstepPager) applyOrder(query *ScheduledStepQuery) *ScheduledStepQuery {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	if p.order.Field != DefaultScheduledStepOrder.Field {
		query = query.Order(DefaultScheduledStepOrder.Field.toTerm(direction.OrderTermOption()))
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(p.order.Field.column)
	}
...
func (p *scheduledstepPager) orderExpr(query *ScheduledStepQuery) sql.Querier {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.column).Pad().WriteString(string(direction))
		if p.order.Field != DefaultScheduledStepOrder.Field {
			b.Comma().Ident(DefaultScheduledStepOrder.Field.column).Pad().WriteString(string(direction))
		}
	})
}
...
func (ss *ScheduledStepQuery) Paginate(
	...
	pager, err := newScheduledStepPager(opts, last != nil)
	if err != nil {
		return nil, err
	}
...
type ScheduledStepOrderField struct {
	// Value extracts the ordering value from the given ScheduledStep.
	Value    func(*ScheduledStep) (ent.Value, error)
	column   string // field or computed.
	toTerm   func(...sql.OrderTermOption) scheduledstep.OrderOption
...
var DefaultScheduledStepOrder = &ScheduledStepOrder{
	...
	Field: &ScheduledStepOrderField{
		Value: func(ss *ScheduledStep) (ent.Value, error) {
			return ss.ID, nil
		},
		column: scheduledstep.FieldID,
		toTerm: scheduledstep.ByID,
