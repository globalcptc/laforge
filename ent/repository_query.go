...
type RepositoryQuery struct {
	...
	withNamedRepositoryToEnvironment map[string]*EnvironmentQuery
	withNamedRepositoryToRepoCommit  map[string]*RepoCommitQuery
...
func (rq *RepositoryQuery) QueryRepositoryToRepoCommit() *RepoCommitQuery {
	query := (&RepoCommitClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RepositoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Repository, error) {
	...
	for name, query := range rq.withNamedRepositoryToEnvironment {
		if err := rq.loadRepositoryToEnvironment(ctx, query, nodes,
			func(n *Repository) { n.appendNamedRepositoryToEnvironment(name) },
			func(n *Repository, e *Environment) { n.appendNamedRepositoryToEnvironment(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range rq.withNamedRepositoryToRepoCommit {
		if err := rq.loadRepositoryToRepoCommit(ctx, query, nodes,
			func(n *Repository) { n.appendNamedRepositoryToRepoCommit(name) },
			func(n *Repository, e *RepoCommit) { n.appendNamedRepositoryToRepoCommit(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (rq *RepositoryQuery) loadRepositoryToRepoCommit(ctx context.Context, query *RepoCommitQuery, nodes []*Repository, init func(*Repository), assign func(*Repository, *RepoCommit)) error {
	...
	query.Where(predicate.RepoCommit(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(repository.RepositoryToRepoCommitColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "repository_repository_to_repo_commit" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
// WithNamedRepositoryToEnvironment tells the query-builder to eager-load the nodes that are connected to the "RepositoryToEnvironment"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (rq *RepositoryQuery) WithNamedRepositoryToEnvironment(name string, opts ...func(*EnvironmentQuery)) *RepositoryQuery {
	query := (&EnvironmentClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if rq.withNamedRepositoryToEnvironment == nil {
		rq.withNamedRepositoryToEnvironment = make(map[string]*EnvironmentQuery)
	}
	rq.withNamedRepositoryToEnvironment[name] = query
	return rq
}

// WithNamedRepositoryToRepoCommit tells the query-builder to eager-load the nodes that are connected to the "RepositoryToRepoCommit"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (rq *RepositoryQuery) WithNamedRepositoryToRepoCommit(name string, opts ...func(*RepoCommitQuery)) *RepositoryQuery {
	query := (&RepoCommitClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if rq.withNamedRepositoryToRepoCommit == nil {
		rq.withNamedRepositoryToRepoCommit = make(map[string]*RepoCommitQuery)
	}
	rq.withNamedRepositoryToRepoCommit[name] = query
	return rq
}
