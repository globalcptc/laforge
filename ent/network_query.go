...
type NetworkQuery struct {
	...
	withNamedNetworkToHostDependency  map[string]*HostDependencyQuery
	withNamedNetworkToIncludedNetwork map[string]*IncludedNetworkQuery
...
func (nq *NetworkQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Network, error) {
	...
	for name, query := range nq.withNamedNetworkToHostDependency {
		if err := nq.loadNetworkToHostDependency(ctx, query, nodes,
			func(n *Network) { n.appendNamedNetworkToHostDependency(name) },
			func(n *Network, e *HostDependency) { n.appendNamedNetworkToHostDependency(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range nq.withNamedNetworkToIncludedNetwork {
		if err := nq.loadNetworkToIncludedNetwork(ctx, query, nodes,
			func(n *Network) { n.appendNamedNetworkToIncludedNetwork(name) },
			func(n *Network, e *IncludedNetwork) { n.appendNamedNetworkToIncludedNetwork(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (nq *NetworkQuery) loadNetworkToHostDependency(ctx context.Context, query *HostDependencyQuery, nodes []*Network, init func(*Network), assign func(*Network, *HostDependency)) error {
	...
	query.Where(predicate.HostDependency(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(network.NetworkToHostDependencyColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "host_dependency_host_dependency_to_network" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (nq *NetworkQuery) loadNetworkToIncludedNetwork(ctx context.Context, query *IncludedNetworkQuery, nodes []*Network, init func(*Network), assign func(*Network, *IncludedNetwork)) error {
	...
	query.Where(predicate.IncludedNetwork(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(network.NetworkToIncludedNetworkColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "included_network_included_network_to_network" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
// WithNamedNetworkToHostDependency tells the query-builder to eager-load the nodes that are connected to the "NetworkToHostDependency"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (nq *NetworkQuery) WithNamedNetworkToHostDependency(name string, opts ...func(*HostDependencyQuery)) *NetworkQuery {
	query := (&HostDependencyClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if nq.withNamedNetworkToHostDependency == nil {
		nq.withNamedNetworkToHostDependency = make(map[string]*HostDependencyQuery)
	}
	nq.withNamedNetworkToHostDependency[name] = query
	return nq
}

// WithNamedNetworkToIncludedNetwork tells the query-builder to eager-load the nodes that are connected to the "NetworkToIncludedNetwork"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (nq *NetworkQuery) WithNamedNetworkToIncludedNetwork(name string, opts ...func(*IncludedNetworkQuery)) *NetworkQuery {
	query := (&IncludedNetworkClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if nq.withNamedNetworkToIncludedNetwork == nil {
		nq.withNamedNetworkToIncludedNetwork = make(map[string]*IncludedNetworkQuery)
	}
	nq.withNamedNetworkToIncludedNetwork[name] = query
	return nq
}
