...
type UserQuery struct {
	...
	withNamedTag          map[string]*TagQuery
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	for name, query := range uq.withNamedTag {
		if err := uq.loadTag(ctx, query, nodes,
			func(n *User) { n.appendNamedTag(name) },
			func(n *User, e *Tag) { n.appendNamedTag(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range uq.withNamedEnvironments {
		if err := uq.loadEnvironments(ctx, query, nodes,
			func(n *User) { n.appendNamedEnvironments(name) },
			func(n *User, e *Environment) { n.appendNamedEnvironments(name, e) }); err != nil {
			return nil, err
		}
	}
...
// WithNamedTag tells the query-builder to eager-load the nodes that are connected to the "Tag"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithNamedTag(name string, opts ...func(*TagQuery)) *UserQuery {
	query := (&TagClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if uq.withNamedTag == nil {
		uq.withNamedTag = make(map[string]*TagQuery)
	}
	uq.withNamedTag[name] = query
	return uq
}

// WithNamedEnvironments tells the query-builder to eager-load the nodes that are connected to the "Environments"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithNamedEnvironments(name string, opts ...func(*EnvironmentQuery)) *UserQuery {
	query := (&EnvironmentClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if uq.withNamedEnvironments == nil {
		uq.withNamedEnvironments = make(map[string]*EnvironmentQuery)
	}
	uq.withNamedEnvironments[name] = query
	return uq
}
