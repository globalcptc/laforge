...
func (auq *AuthUserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AuthUser, error) {
	...
	for name, query := range auq.withNamedTokens {
		if err := auq.loadTokens(ctx, query, nodes,
			func(n *AuthUser) { n.appendNamedTokens(name) },
			func(n *AuthUser, e *Token) { n.appendNamedTokens(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range auq.withNamedServerTasks {
		if err := auq.loadServerTasks(ctx, query, nodes,
			func(n *AuthUser) { n.appendNamedServerTasks(name) },
			func(n *AuthUser, e *ServerTask) { n.appendNamedServerTasks(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (auq *AuthUserQuery) loadTokens(ctx context.Context, query *TokenQuery, nodes []*AuthUser, init func(*AuthUser), assign func(*AuthUser, *Token)) error {
	...
	query.Where(predicate.Token(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(authuser.TokensColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "auth_user_tokens" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (auq *AuthUserQuery) loadServerTasks(ctx context.Context, query *ServerTaskQuery, nodes []*AuthUser, init func(*AuthUser), assign func(*AuthUser, *ServerTask)) error {
	...
	query.Where(predicate.ServerTask(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(authuser.ServerTasksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "server_task_auth_user" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (auq *AuthUserQuery) WithNamedTokens(name string, opts ...func(*TokenQuery)) *AuthUserQuery {
	query := (&TokenClient{config: auq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if auq.withNamedTokens == nil {
		auq.withNamedTokens = make(map[string]*TokenQuery)
	}
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (auq *AuthUserQuery) WithNamedServerTasks(name string, opts ...func(*ServerTaskQuery)) *AuthUserQuery {
	query := (&ServerTaskClient{config: auq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if auq.withNamedServerTasks == nil {
		auq.withNamedServerTasks = make(map[string]*ServerTaskQuery)
	}
	auq.withNamedServerTasks[name] = query
	return auq
}
