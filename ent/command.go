...
type CommandEdges struct {
	...
	namedCommandToUser map[string][]*User
}
...
func (c *Command) QueryCommandToUser() *UserQuery {
	return NewCommandClient(c.config).QueryCommandToUser(c)
}
...
func (c *Command) QueryCommandToEnvironment() *EnvironmentQuery {
	return NewCommandClient(c.config).QueryCommandToEnvironment(c)
}
...
// NamedCommandToUser returns the CommandToUser named value or an error if the edge was not
// loaded in eager-loading with this name.
func (c *Command) NamedCommandToUser(name string) ([]*User, error) {
	if c.Edges.namedCommandToUser == nil {
		return nil, &NotLoadedError{edge: name}
	}
	nodes, ok := c.Edges.namedCommandToUser[name]
	if !ok {
		return nil, &NotLoadedError{edge: name}
	}
	return nodes, nil
...
func (c *Command) appendNamedCommandToUser(name string, edges ...*User) {
	if c.Edges.namedCommandToUser == nil {
		c.Edges.namedCommandToUser = make(map[string][]*User)
	}
	if len(edges) == 0 {
		c.Edges.namedCommandToUser[name] = []*User{}
	} else {
		c.Edges.namedCommandToUser[name] = append(c.Edges.namedCommandToUser[name], edges...)
	}
}
