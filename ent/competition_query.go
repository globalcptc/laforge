...
type CompetitionQuery struct {
	...
	withNamedCompetitionToDNS    map[string]*DNSQuery
	withNamedCompetitionToBuild  map[string]*BuildQuery
...
func (cq *CompetitionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Competition, error) {
	...
	if query := cq.withCompetitionToDNS; query != nil {
		if err := cq.loadCompetitionToDNS(ctx, query, nodes,
			func(n *Competition) { n.Edges.CompetitionToDNS = []*DNS{} },
			func(n *Competition, e *DNS) { n.Edges.CompetitionToDNS = append(n.Edges.CompetitionToDNS, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withCompetitionToEnvironment; query != nil {
		if err := cq.loadCompetitionToEnvironment(ctx, query, nodes, nil,
			func(n *Competition, e *Environment) { n.Edges.CompetitionToEnvironment = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withCompetitionToBuild; query != nil {
		if err := cq.loadCompetitionToBuild(ctx, query, nodes,
			func(n *Competition) { n.Edges.CompetitionToBuild = []*Build{} },
			func(n *Competition, e *Build) { n.Edges.CompetitionToBuild = append(n.Edges.CompetitionToBuild, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range cq.withNamedCompetitionToDNS {
		if err := cq.loadCompetitionToDNS(ctx, query, nodes,
			func(n *Competition) { n.appendNamedCompetitionToDNS(name) },
			func(n *Competition, e *DNS) { n.appendNamedCompetitionToDNS(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range cq.withNamedCompetitionToBuild {
		if err := cq.loadCompetitionToBuild(ctx, query, nodes,
			func(n *Competition) { n.appendNamedCompetitionToBuild(name) },
			func(n *Competition, e *Build) { n.appendNamedCompetitionToBuild(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (cq *CompetitionQuery) loadCompetitionToDNS(ctx context.Context, query *DNSQuery, nodes []*Competition, init func(*Competition), assign func(*Competition, *DNS)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(competition.CompetitionToDNSTable)
		s.Join(joinT).On(s.C(dns.FieldID), joinT.C(competition.CompetitionToDNSPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(competition.CompetitionToDNSPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(competition.CompetitionToDNSPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "CompetitionToDNS" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (cq *CompetitionQuery) loadCompetitionToEnvironment(ctx context.Context, query *EnvironmentQuery, nodes []*Competition, init func(*Competition), assign func(*Competition, *Environment)) error {
...
func (cq *CompetitionQuery) loadCompetitionToBuild(ctx context.Context, query *BuildQuery, nodes []*Competition, init func(*Competition), assign func(*Competition, *Build)) error {
	...
	query.Where(predicate.Build(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(competition.CompetitionToBuildColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "build_build_to_competition" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
// WithNamedCompetitionToDNS tells the query-builder to eager-load the nodes that are connected to the "CompetitionToDNS"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (cq *CompetitionQuery) WithNamedCompetitionToDNS(name string, opts ...func(*DNSQuery)) *CompetitionQuery {
	query := (&DNSClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if cq.withNamedCompetitionToDNS == nil {
		cq.withNamedCompetitionToDNS = make(map[string]*DNSQuery)
	}
	cq.withNamedCompetitionToDNS[name] = query
	return cq
}

// WithNamedCompetitionToBuild tells the query-builder to eager-load the nodes that are connected to the "CompetitionToBuild"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (cq *CompetitionQuery) WithNamedCompetitionToBuild(name string, opts ...func(*BuildQuery)) *CompetitionQuery {
	query := (&BuildClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if cq.withNamedCompetitionToBuild == nil {
		cq.withNamedCompetitionToBuild = make(map[string]*BuildQuery)
	}
	cq.withNamedCompetitionToBuild[name] = query
	return cq
}
